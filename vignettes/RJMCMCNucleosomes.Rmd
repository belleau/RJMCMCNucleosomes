...
sample <- nucleoSim::syntheticNucReadsFromDist(wp.num=val.num,
                                    ...
                                    max.cover=val.max.cover, nuc.len=val.nuc.len,
                                    len.var=val.len.var, lin.len=val.lin.len,
...
readsForward <- start(segment01[strand(segment01) == "+"])
readsReverse <- end(segment01[strand(segment01) == "-"])
...
directory <- system.file("extdata", "demo_vignette", package = "RJMCMCNucleosomes")
...
resultsPostTreatment <- postTreatment(startPosForwardReads =  
                                allReadsForward$start,
...
to predict more than one set of nucleosome positions (as example, before and
after post-treatment or results from different software), the predictions can
be merged in a \code{list} so that all predictions can be plot simultaneously.
...
resultsBeforeAndAfter <- list(Sample = c(sample$wp$nucleopos, sample$fuz$nucleopos),
