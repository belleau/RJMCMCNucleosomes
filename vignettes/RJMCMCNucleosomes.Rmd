...
The `postTreatment` function can significantly reduces the number of 
...
The `plotNucleosomes` function needs the nucleosome positions as well as 
the reads, in an `IRanges` format, to create a graph. When reads are used 
...
The \code{rjmcmcCHR} can analyse an entire chromosome by automatically 
split the reads into segments, run \code{rjmcmc} on each segment, merge and
post-process the results. The intermediary steps are conserved in a 
directory set by the \code{dirOut} parameter.
...
result <- rjmcmcCHR(forwardandReverseReads=sampleGRanges, 
...
during the segmentation step are saved in RDS files.
