...
Region segmentation can be done using the \code{segmentation} function.
...
## Run RJMCMCNucleosomes

The \code{rjmcmc} function must be run on each candidate region. As an example, the 
...
```{r visualisation, collapse=TRUE, message=FALSE, fig.height=6.5}
...
resultsBeforeAndAfter <- list(BeforePostTreatment=resultsAllMerged$mu, 
                              AfterPostTreatment=resultsPostTreatment)

## Create graph using nucleosome positions and reads
plotNucleosomes(resultsBeforeAndAfter, reads, names=c("Before", "After"))
```
